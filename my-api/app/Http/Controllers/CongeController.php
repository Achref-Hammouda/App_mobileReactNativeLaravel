...
class CongeController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($superior) {
            ...
            $recipients = $superior->pushTokens->pluck('token')->toArray();
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($conge->etat === 'attent') {
            ...
            $recipients = $superior->pushTokens->pluck('token')->toArray();
            $title = "{$user->nom} {$user->prenom}";
            $body = "a modifier un congé";
            $expoService->sendPushNotification($recipients, $title, $body);
            return response()->json(['message' => 'congé modifier avec success']);
        }
        return response()->json(['message' => "ce congé est traitté"]);
    }

    public function getByUserId($user_id)
    {
        $conges = Conge::where('user_id', $user_id)->with('user:id,email,solde_de_conge')->get();
        return response()->json($conges);
    }
    ...
    public function destroy($id)
    {
        ...
        if ($conge->etat === 'attent') {
            ...
            $recipients = $superior->pushTokens->pluck('token')->toArray();
            $title = "{$user->nom} {$user->prenom}";
            $body = "a supprimer un congé";
            $expoService->sendPushNotification($recipients, $title, $body);
            return response()->json(['message' => 'congé supprimer avec success']);
        }
        return response()->json(['message' => "ce congé est traitté"]);
    }
}
